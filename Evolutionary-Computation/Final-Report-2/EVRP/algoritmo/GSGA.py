...
class GSGA():
    """
    Aqui está uma implementação básica de uma classe GA em Python para resolver o problema VRP.
    Ele pega um objeto `Problem`, tamanho da população, gerações, probabilidade de crossover, probabilidade de mutação e tamanho da elite como argumentos de entrada.
    Ele tem um método `run` que retorna a melhor solução encontrada pelo GA após o número especificado de gerações.
    Ele também tem várias funções auxiliares para inicializar a população, obter a elite, seleção de torneio, crossover e mutação.
    Observe que esta implementação fornece apenas uma estrutura básica para um GA e pode precisar ser modificada ou estendida dependendo do problema VRP específico que você está tentando resolver.
    ...
    def solve(self, problem: Problem, verbose=False, plot_path=None) -> Solution:
        ...
        for i in range(self.generations):
            ...
            max_fit = np.max([indv.get_tour_length() for i, indv in enumerate(self.population) if valids[i]])
            std_fit = np.std([indv.get_tour_length() for i, indv in enumerate(self.population) if valids[i]])

            if verbose:
                print(f"Geração: {i}, Fitness Medio: {np.round(mean_fit, 3)}, Fitness Minimo: {np.round(best_fit, 3)}, Fitness Máximo: {np.round(max_fit, 3)}, Desvio Padrão: {np.round(std_fit, 3)}, Alpha: {np.round(alpha, 3)}")
                
            self.history['Fitness Medio'].append(mean_fit)
            self.history['Melhor Fitness'].append(best_fit)
        ...
        best_individual_index = np.argmin([indv.get_tour_length() for indv in self.population])
        best_individual = self.population[best_individual_index]
        final_valids = [indv.get_tour_length() for indv in self.population if self.problem.check_valid_solution(indv)]
            
        final_mean_fit = np.mean(final_valids)
        final_best_fit = np.min(final_valids)
        final_max_fit = np.max(final_valids)
        final_std_fit = np.std(final_valids)
            
        return best_individual, final_mean_fit, final_best_fit, final_max_fit, final_std_fit
    ...
    def plot_history(self, path):
        ...
        plt.xlabel('Geração')
        plt.ylabel('Fitness')
        plt.title('Tendências de convergência ({})'.format(self.problem.get_name()))
        plt.legend()
        plt.grid()
        plt.savefig(path)
        plt.close()
